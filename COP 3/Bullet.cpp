#include <bits/stdc++.h>


//TODO : add other haeder files , FOR OTHER FILES , DO THE SAME THING TOO

#include "Bullet.h"
#include "Vector.h"
#include "Walls.h"


Bullet::Bullet()
{
	cout<<"Bullet created successfully"<<endl;
}

Bullet::~Bullet()
{
	cout<<"Bullet deleted successfully"<<endl;
}

void Bullet::setPosition(Vector pos)
{
	Position = pos;
}

void Bullet::setType(string type)
{
	Type = type;
}

void Bullet::setBulletID(string id)
{
	BulletID = id;
}

void Bullet::setUserID(string user)
{
	UserID = user;
}

void Bullet::setVelocity(Vector vel)
{
	Velocity = vel;
}

void Bullet::setDirection(string dire)
{
	Direction = dire;
}

void Bullet::setDeathStatus(bool life)
{
	DeathStatus = life;
}

Vector Bullet::getPosition()
{
	return Position;
}

string Bullet::getType()
{
	return Type;
}

string Bullet::getBulletID()
{
	return BulletID;
}

string Bullet::getUserID()
{
	return UserID;
}

Vector Bullet::getVelocity()
{
	return Velocity;
}

bool getDeathStatus()
{
	return DeathStatus;
}

void Bullet::Move(string dire , Vector change)
{
	if(Direction == dire)
	{
		Position = Position.AddVector(change);
	}
	else 
	{
		Direction = dire;
	}
}

bool CollideWithTanks(vector<Tank> tanks)
{
	for(int i=0;i<tanks.size();i++)
	{
		if(abs(Position - tanks[i].getCentre())<dis)
		{
			DeathStatus = true;
			return true;
		}
	}
	return false;
}

bool Bullet::CollideWithWalls(int* BrickFilledRows , int Rows , int Columns)
{
	/*TODO : modify the maths part . Area A divided into m*n square for walls
			for a position (x,y), it lies inside the square number (x mod n , y mod m)
	*/

	if(Direction=="up")
	{
		if(Position.getY()==0)
		{
			DeathStatus = true;
			return true;
		}

	}

	else if(Direction=="down")
	{
		if((Position.getY()%Rows)==Rows - 1)
		{
			DeathStatus = true;
			return true;
		}
	}

	else if(Direction=="right")
	{
		if(Position.getX()==Columns-1)
		{
			DeathStatus = true;
			return true;
		}
	}

	else if(Direction == "left")
	{
		if(Position.getX()==0)
		{
			DeathStatus = true;
			return true;
		}
	}

	else if(BrickFilled[(Position.getX()%Columns)*(Position.getY()%Rows)]!=0)
	{
		DeathStatus = true;
		return true;
	}	
	
	else return false;
}

bool Bullet::CollideWithBullet(vector<Bullet> bullets)
{
	for(int i=0;i<bullets.size();i++)
	{
		if((abs(Position - bullets[i].getPosition())< dis) && (BulletID != bullets[i].getBulletID()))
		{
			//bullet collides with another bullet i
			DeathStatus= true;
			return true;
		}
	}
	return false;
}
